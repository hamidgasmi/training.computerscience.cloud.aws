...
- Client-side Encryption: 
	- It's the responsibility of the client/application:
		- Encryption/decryption process (methods)
		- Encryption keys
...
- Encryption At Rest:
	...
	- Sever-Side Encryption with Customer-Managed Keys (SSE-C):
		- S3 handles the encryption/decryption process
		- The customer is responsible for key management
		- Keys must be supplied with each PUT/GET request
	- Sever-Side Encryption with S3-Managed Keys (SSE-S3):
		- S3 encrypts object by using AES-256
		- Keys are generated by S3 using AWS KMS
		- Keys are stored with objects in an encrypted form
		- Principals who have permission to the object can decrypt it and access it 
	- Sever-Side Encryption with AWS KMS-Managed Keys (SSE-KMS):
		...
		- Decryption of an object needs both S3 and KMS key permission (role separation)
